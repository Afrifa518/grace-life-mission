...
const MinistryRegistrations = () => {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      if (search) {
        query = query.or(
          `ministry_title.ilike.%${search}%,name.ilike.%${search}%,email.ilike.%${search}%,message.ilike.%${search}%`
        );
      }
      ...
      toast({ title: 'Error fetching registrations', description: err.message, variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold text-foreground">Ministry Registrations</h2>
        <p className="text-muted-foreground mt-2">Ministry sign-up submissions from the Ministries page.</p>
      </div>
      ...
      {viewing && (
        <Modal open={true} title={`Registration: ${viewing.ministry_title || ''}`} onClose={() => setViewing(null)}>
